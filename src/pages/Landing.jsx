  ...
  return (
    <div style={{ fontFamily: "var(--font-body)", color: "var(--text)" }}>
      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", padding: "120px 24px 80px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle,rgba(124,58,237,0.15) 0%,transparent 70%)", top: -100, left: -100, pointerEvents: "none" }} />
        <div style={{ position: "absolute", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle,rgba(0,229,255,0.1) 0%,transparent 70%)", bottom: -100, right: -100, pointerEvents: "none" }} />
        ...
        <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(30px)", transition: "all 0.7s ease", maxWidth: 760, position: "relative", zIndex: 1 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 16px", borderRadius: 20, border: "1px solid rgba(124,58,237,0.4)", background: "rgba(124,58,237,0.1)", marginBottom: 28 }}>
            <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--cyan)" }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: "var(--cyan)", fontFamily: "var(--font-mono)", letterSpacing: "0.08em" }}>CLASSES 6 – 10 · NCERT CURRICULUM · BENGALURU</span>
          </div>

          <h1 style={{ fontSize: "clamp(42px,7vw,80px)", fontWeight: 700, lineHeight: 1.05, letterSpacing: "-0.03em", marginBottom: 24, fontFamily: "var(--font-head)" }}>
            Learn the way<br />
            <span style={{ background: "var(--grad)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>your brain works</span>
          </h1>

          <p style={{ fontSize: "clamp(15px,2vw,18px)", color: "var(--text2)", lineHeight: 1.7, maxWidth: 540, margin: "0 auto 16px", fontWeight: 400 }}>
          ...
          <p style={{ fontSize: 13, color: "var(--purple3)", fontWeight: 600, margin: "0 auto 36px", fontFamily: "var(--font-mono)" }}>
            ✦ Premium students get AI-powered homework assistance — upload a photo and get step-by-step solutions
          </p>

          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            {user ? (
              <button className="grad-btn" style={{ padding: "14px 32px", fontSize: 16, borderRadius: 14 }} onClick={() => setPage(profile?.role === "teacher" ? "teacher" : "dashboard")}>
                Go to Dashboard
              </button>
            ) : (
              <>
                <button className="grad-btn" style={{ padding: "14px 32px", fontSize: 16, borderRadius: 14 }} onClick={() => setPage("signup")}>Start Learning Free</button>
                <button onClick={() => setPage("login")} style={{ padding: "14px 32px", fontSize: 16, borderRadius: 14, border: "1px solid var(--border2)", background: "transparent", color: "var(--text)", cursor: "pointer", fontFamily: "var(--font-body)", fontWeight: 600 }}>Sign In</button>
              </>
            )}
          </div>

          <div style={{ display: "flex", gap: 20, justifyContent: "center", marginTop: 52, flexWrap: "wrap" }}>
            {[["5 Classes", "6th to 10th"], ["4 Subjects", "NCERT Based"], ["4–8 PM", "Mon to Fri"], ["Rs.200", "Per Week"]].map(([val, label]) => (
              <div key={val} style={{ padding: "10px 20px", borderRadius: 12, border: "1px solid var(--border)", background: "var(--surface)" }}>
                <p style={{ margin: 0, fontSize: 16, fontWeight: 700, fontFamily: "var(--font-head)", color: "var(--text)" }}>{val}</p>
                <p style={{ margin: 0, fontSize: 11, color: "var(--text3)", fontWeight: 500, marginTop: 2 }}>{label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="features" style={{ padding: "100px 24px", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          <p style={{ fontSize: 12, fontWeight: 600, color: "var(--purple3)", fontFamily: "var(--font-mono)", letterSpacing: "0.1em", marginBottom: 12 }}>WHY SAINT ACADEMY</p>
          <h2 style={{ fontSize: "clamp(28px,4vw,42px)", fontWeight: 700, letterSpacing: "-0.02em", fontFamily: "var(--font-head)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 20 }}>
          {[
            ...
            <div key={f.num} className="card" style={{ padding: "28px 26px" }}>
              ...
              <h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 10, fontFamily: "var(--font-head)", letterSpacing: "-0.01em" }}>{f.title}</h3>
      ...
      {/* ── TIMETABLE ── */}
      <section id="schedule" style={{ padding: "80px 24px", maxWidth: 1000, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <p style={{ fontSize: 12, fontWeight: 600, color: "var(--purple3)", fontFamily: "var(--font-mono)", letterSpacing: "0.1em", marginBottom: 12 }}>WEEKLY SCHEDULE</p>
          <h2 style={{ fontSize: "clamp(24px,3vw,36px)", fontWeight: 700, fontFamily: "var(--font-head)", letterSpacing: "-0.02em", marginBottom: 8 }}>Monday to Friday · 4 PM – 8 PM</h2>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 28 }}>
            {["6", "7", "8", "9", "10"].map(c => (
              <button key={c} onClick={() => setTtClass(c)} style={{ padding: "7px 18px", borderRadius: 20, border: "none", cursor: "pointer", fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 700, background: ttClass === c ? "var(--grad)" : "var(--surface)", color: ttClass === c ? "#fff" : "var(--text2)", border: ttClass === c ? "none" : "1px solid var(--border)", transition: "all 0.2s" }}>
                Class {c}
              </button>
            ))}
          </div>
      ...
      {/* ── PRICING ── */}
      <section id="pricing" style={{ padding: "100px 24px", maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: "var(--purple3)", fontFamily: "var(--font-mono)", letterSpacing: "0.1em", marginBottom: 12 }}>PRICING</p>
        <h2 style={{ fontSize: "clamp(28px,4vw,42px)", fontWeight: 700, fontFamily: "var(--font-head)", letterSpacing: "-0.02em", marginBottom: 40 }}>
          {profile?.role === "teacher" ? "Platform Access" : "Simple, affordable."}
        </h2>

        {profile?.role === "teacher" ? (
          /* TEACHER */
          <div style={{ background: "var(--bg2)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 24, padding: "40px 36px" }}>
          ...
          <div style={{ background: "var(--bg2)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 24, padding: "40px 36px", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: "var(--grad)" }} />
            <div style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg,rgba(124,58,237,0.2),rgba(0,229,255,0.2))", border: "1px solid rgba(124,58,237,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 22 }}>✦</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, textAlign: "left", maxWidth: 500, margin: "0 auto" }}>
              {["Full NCERT study material, chapter-wise", "Teacher's premium notes per subject", "Homework assigned chapter-by-chapter", "AI assistant — upload photos of homework", "Raise doubts directly to subject teacher", "Live Google Meet classes with Join button", "Class-specific weekly timetable", "Priority support from your teacher"].map(f => (
                <div key={f} style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                  <div style={{ width: 16, height: 16, borderRadius: "50%", background: "rgba(0,229,255,0.15)", border: "1px solid rgba(0,229,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
          ...
          <div style={{ maxWidth: 420, margin: "0 auto" }}>
            <div style={{ background: "var(--bg2)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 24, padding: "32px 28px", textAlign: "left", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: "var(--grad)" }} />
              <div style={{ position: "absolute", width: 200, height: 200, borderRadius: "50%", background: "radial-gradient(circle,rgba(124,58,237,0.1) 0%,transparent 70%)", top: -60, right: -60, pointerEvents: "none" }} />
              <div style={{ display: "inline-block", padding: "3px 12px", borderRadius: 20, background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.3)", marginBottom: 12 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: "var(--purple3)", fontFamily: "var(--font-mono)" }}>UPGRADE TO PREMIUM</span>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
                {["NCERT Textbooks & Solutions", "Teacher's chapter-wise notes", "Homework & assignments", "AI assistant — upload homework photos", "Join live Google Meet classes", "Raise doubts to subject teacher", "Personal teacher remarks", "Weekly tests & mock tests"].map(f => (
                  <div key={f} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 14, height: 14, borderRadius: "50%", background: "rgba(0,229,255,0.15)", border: "1px solid rgba(0,229,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <button className="grad-btn" style={{ width: "100%", padding: "13px", fontSize: 15, borderRadius: 12 }} onClick={() => setShowPayment(true)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, maxWidth: 800, margin: "0 auto" }}>
            {/* Free plan */}
            <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 24, padding: "32px 28px", textAlign: "left" }}>
              ...
              <button onClick={() => setPage("signup")} style={{ width: "100%", padding: "12px", borderRadius: 12, border: "1px solid var(--border)", background: "transparent", color: "var(--text2)", fontFamily: "var(--font-body)", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
                Start Free
              </button>
            </div>
            ...
            <div style={{ background: "var(--bg2)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 24, padding: "32px 28px", textAlign: "left", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "absolute", width: 200, height: 200, borderRadius: "50%", background: "radial-gradient(circle,rgba(124,58,237,0.1) 0%,transparent 70%)", top: -60, right: -60, pointerEvents: "none" }} />
              <div style={{ display: "inline-block", padding: "3px 12px", borderRadius: 20, background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.3)", marginBottom: 12 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: "var(--purple3)", fontFamily: "var(--font-mono)" }}>MOST POPULAR</span>
              </div>
              <div style={{ marginBottom: 8 }}>
                <span style={{ fontSize: 36, fontWeight: 700, fontFamily: "var(--font-head)", background: "var(--grad)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Rs.200</span>
                <span style={{ fontSize: 14, color: "var(--text3)", marginLeft: 6 }}>/week</span>
              ...
              <button className="grad-btn" style={{ width: "100%", padding: "13px", fontSize: 15, borderRadius: 12 }} onClick={() => setPage("signup")}>
                Get Started Now
              </button>
            </div>
          </div>
      ...
      {/* ── CONTACT ── */}
      <section id="contact" style={{ padding: "80px 24px 120px", maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: "var(--purple3)", fontFamily: "var(--font-mono)", letterSpacing: "0.1em", marginBottom: 12 }}>CONTACT</p>
        <h2 style={{ fontSize: "clamp(24px,3vw,36px)", fontWeight: 700, fontFamily: "var(--font-head)", letterSpacing: "-0.02em", marginBottom: 20 }}>Got questions?</h2>
        ...
        <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
          <a href="mailto:saintacademy@email.com" style={{ padding: "12px 24px", borderRadius: 12, border: "1px solid var(--border2)", background: "var(--surface)", color: "var(--cyan)", fontSize: 14, fontWeight: 600, textDecoration: "none" }}>saintacademy@email.com</a>
          <a href="tel:+918318081890" style={{ padding: "12px 24px", borderRadius: 12, border: "1px solid var(--border2)", background: "var(--surface)", color: "var(--text)", fontSize: 14, fontWeight: 600, textDecoration: "none" }}>+91 83180 81890</a>
        </div>
      </section>
